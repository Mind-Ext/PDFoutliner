...
let _promise_id = 1
let _promise_map = new Map()

const wrap = function (name) {
  return function (...args) {
    return new Promise(function (resolve, reject) {
      let id = _promise_id++
      _promise_map.set(id, { resolve, reject })
...
worker.onmessage = function (event) {
  ...
  switch (type) {
    case 'INIT':
      for (let method of result) worker[method] = wrap(method)
      show_message('Open or drop PDF here')
      initDropzone()
      break

    case 'RESULT':
      _promise_map.get(id).resolve(result)
      _promise_map.delete(id)
      break
    ...
    case 'ERROR':
      ...
      _promise_map.get(id).reject(error)
      _promise_map.delete(id)
      break
...
class PageView {
  constructor(doc, pageNumber, defaultSize, zoom) {
    ...
    this.rootNode.className = 'pdfpage'
...
function zoom_width() {
  ...
  const pxWidth = panel.clientWidth - 24 * 2 // 24px is margin
  const pageWidth = page_list[0].size.width / 72
  zoom_to((pxWidth / pageWidth / 96) * 100)
}

function zoomWidthFirstPage() {
  // 100 = 96dpi
  const panel = document.getElementById('page-panel')
  const pxWidth = panel.clientWidth - 24 * 2 // 24px is margin
  const pageWidth = page_list[0].size.width / 72
  const zoom = (pxWidth / pageWidth / 96) * 100
  current_zoom = zoom
  for (let page of page_list) page.setZoom(zoom)
  queue_update_view()
}
...
function toggle_search_panel(state = null) {
  return
  // if (state === true || search_panel.style.display === 'none') {
  //   // show
  //   search_panel.style.display = 'flex'
  //   search_input.focus()
  //   search_input.select()
  // } else if (state === false || search_panel.style.display !== 'none') {
  //   // hide
...
async function open_document_from_buffer(buffer, magic, title) {
  ...
  var page_size = await worker.getPageSize(current_doc, 0)
...
async function open_document_from_file(file) {
  ...
  try {
    ...
    window.store.docTitle = file.name
...
async function handleDrop(event) {
  ...
  if (files.length === 1 && files[0].type === 'application/pdf') {
    await open_document_from_file(files[0])
  } else {
    alert('Please drop only a single PDF file.')
  }
}
